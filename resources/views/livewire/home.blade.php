                                ...
                                <div class="sort-by-dropdown">
                                    <ul>
                                        <li><a class="{{ $sort === 'latest' ? 'active' : '' }}"
                                               href="{{ url()->current() }}?{{ http_build_query(array_merge(request()->query(), ['sort' => 'latest'])) }}">
                                            Новинки
                                        </a></li>
                                        <li><a class="{{ $sort === 'low-to-high' ? 'active' : '' }}"
                                               href="{{ url()->current() }}?{{ http_build_query(array_merge(request()->query(), ['sort' => 'low-to-high'])) }}">
                                            Цена: по возрастанию
                                        </a></li>
                                        <li><a class="{{ $sort === 'high-to-low' ? 'active' : '' }}"
                                               href="{{ url()->current() }}?{{ http_build_query(array_merge(request()->query(), ['sort' => 'high-to-low'])) }}">
                                            Цена: по убыванию
                                        </a></li>
                                        <li><a href="{{ route('home') }}">Сортировка по умолчанию</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="show flex justify-end mt-3">
                                                <form action="{{ route('cart.add') }}" method="post">
                                                    ...
                                                    <a onclick="this.closest('form').submit()" class="text-orange-500 hover:text-black duration-100" title="Добавить в корзину">
